...
public class TestDownloadController extends ModuleDownloadController{
	...
	public ModelAndView download(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		if(fidx > 0) {
			...
			dt = testService.getMultiFileView(fnIdx, param);
			if(dt != null) {
				int result = testService.updateMultiFileDown(fnIdx, keyIdx, fidx, itemId);
			}
		} else {
			...
			param.put("searchList", searchList);
			dt = testService.getFileView(fnIdx, param);
			if(dt != null) {
				int result = testService.updateFileDown(fnIdx, keyIdx, columnId);
			}
		}
